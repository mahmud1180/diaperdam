"""GoBaby diaper scraper. WooCommerce Store API (no auth)."""
import asyncio
import json
import logging
import re
            ...
            for q in ["diaper", "huggies", "mamypoko", "molfix"]:
                ...
                while page <= 5:
                    try:
                        ...
                        if not items: break
                        for item in items:
                            p = self._parse_wc(item)
                            if p and p.external_id not in seen:
                                results.append(p); seen.add(p.external_id)
                        if len(items) < 50: break
                        page += 1
                    except Exception as e:
                        logger.warning(f"[gobaby] WC API: {e}"); break
                    await asyncio.sleep(0.5)

            # Fallback: category page HTML
            if not results:
                try:
                    r = await client.get(f"{BASE}/product-category/baby-diaper/", headers=HEADERS)
                            ...
                            try:
                                ...
                                if ld.get("@type") == "ItemList":
                                    for item in ld.get("itemListElement", []):
                                        p = self._parse_ld(item)
                                        if p and p.external_id not in seen:
                                            results.append(p); seen.add(p.external_id)
                            except: continue
                except Exception as e:
                    logger.warning(f"[gobaby] category: {e}")

        logger.info(f"[gobaby] Scraped {len(results)} products")
        return results

    def _parse_wc(self, item):
        try:
            ...
            if original and original <= price: original = None
...
async def main():
    ...
    s = GoBabyScraper(); p = await s.scrape()
    if p: s.upsert_to_db(p)
    print(f"Done: {len(p)} from GoBaby")
